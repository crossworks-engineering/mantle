...
export async function POST(req: Request) {
  ...
  if (!agentSlug) {
    return NextResponse.json(
      {
        error:
          'No Toolsmith is set up yet. Run `pnpm -C server/web seed:toolsmith` (or finish onboarding) to provision the default API-integration specialist, or pick another agent in the panel.',
      },
      { status: 409 },
    );
  }
