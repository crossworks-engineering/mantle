...
server.tool(
  ...
  async ({ path }) => {
    const rows = await db
      ...
      .from(nodes)
      .where(
        and(eq(nodes.ownerId, ownerId), path ? (eq as any)(nodes.path, path) : eq(nodes.type, 'branch')),
      )
      .limit(200);
...
server.tool(
  ...
  async ({ folder_id, path, new_name }) => {
    ...
    try {
      const updated = await renameFolderById({ ownerId: ownerId, folderId: id, newSlug: new_name });
      if (!updated) {
        return { content: [{ type: 'text', text: 'folder not found' }], isError: true };
      }
      return jsonReply(updated);
