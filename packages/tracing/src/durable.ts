 ...
 * in server/api) journal the work already marked by `step()` WITHOUT this package
 * depending on that engine.
 *
 * A caller (the server/api workflow) wraps its work in `withDurableSteps(exec, …)`,
 ...
 *      caller outside a workflow — the web request, server/api, scripts — is
 *      unaffected.
 ...
 *  The server/api workflow uses this to make the turn's steps durable. */
