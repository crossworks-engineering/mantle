...
function physicalTableOf(abs: string): string {
  const db = openTableFile(abs, { readOnly: true });
  try {
    return String(db.prepare(`SELECT physical_table FROM _tabs ORDER BY position LIMIT 1`).get()!.physical_table);
  } finally {
    db.close();
  }
}
...
describe('formula ↔ stored retypes are DDL', () => {
  it('formula → text adds the physical column; the file stays readable', () => {
    ...
    applyOpsToFile(abs, [{ op: 'column_update', columnId: 'c2', patch: { type: 'text', formula: null } }], coerce);
  ...
  it('text → formula drops the physical column and the view survives', () => {
    ...
    applyOpsToFile(abs, [{ op: 'column_update', columnId: 'c2', patch: { type: 'formula', formula: 'c1' } }], coerce);
    const doc = readDocFile(abs);
...
describe('column_update: explicit null clears a property', () => {
  it('clears width, format, options and formula; absent keys keep values', () => {
    ...
    applyOpsToFile(abs, [{ op: 'column_update', columnId: 'c1', patch: { name: 'Quantity' } }], coerce);
    ...
    applyOpsToFile(
      abs,
      [{ op: 'column_update', columnId: 'c1', patch: { width: null, format: null, options: null } }],
      coerce,
    );
...
describe('view names vs the engine namespace', () => {
  it('a tab literally named like a physical table gets a suffixed view instead of failing', () => {
    ...
    const db = openTableFile(abs, { readOnly: true });
    try {
      const views = (db.prepare(`SELECT view_name FROM _tabs ORDER BY position`).all() as unknown as {
        view_name: string;
      }[]).map((v) => v.view_name);
  ...
  it('tab_rename to an engine table name suffixes via uniqueViewName', () => {
    ...
    const tabs = db0.prepare(`SELECT tab_id, physical_table FROM _tabs ORDER BY position`).all() as unknown as {
      tab_id: string;
      physical_table: string;
    }[];
    db0.close();
    const firstPhysical = tabs[0]!.physical_table;
    applyOpsToFile(abs, [{ op: 'tab_rename', tabId: tabs[1]!.tab_id, name: firstPhysical }], coerce);
    const db = openTableFile(abs, { readOnly: true });
    try {
      const renamed = db
