#!/usr/bin/env bash
# Sync this working tree's source to the test box's source-run stack
# (~/mantle-src, served by the docker-compose.dev-src.yml override — see that
# file on the box). Dev servers there hot-reload on sync; no image build.
#
