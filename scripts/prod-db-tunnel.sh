...
# Postgres AND object store — so the local dev server (or a future Electron desktop app)
...
#   PROD_S3_LOCAL_PORT     local port → object store    (default: 9100)
#
...
STORE_CONTAINER="${MANTLE_OBJECTSTORE_CONTAINER:-${MANTLE_MINIO_CONTAINER:-mantle_objectstore}}"
PG_LOCAL_PORT="${PROD_DB_LOCAL_PORT:-55432}"
STORE_LOCAL_PORT="${PROD_S3_LOCAL_PORT:-9100}"
PG_REMOTE_PORT="${PG_REMOTE_PORT:-5432}"
STORE_REMOTE_PORT="${STORE_REMOTE_PORT:-${MINIO_REMOTE_PORT:-9000}}"
ACTION="${1:-up}"
...
case "$ACTION" in
  up)
    if port_open "$PG_LOCAL_PORT"; then
      ...
      port_open "$STORE_LOCAL_PORT" \
        && echo "✓ object store also up on 127.0.0.1:${STORE_LOCAL_PORT}." \
        || echo "• object store port ${STORE_LOCAL_PORT} not up — run 'down' then 'up' to (re)add it."
      exit 0
    fi
    ...
    STORE_IP="$(resolve_ip "$STORE_CONTAINER")"   # optional

    FORWARDS=(-L "127.0.0.1:${PG_LOCAL_PORT}:${PG_IP}:${PG_REMOTE_PORT}")
    echo "→ ${PG_CONTAINER} ${PG_IP}:${PG_REMOTE_PORT} → 127.0.0.1:${PG_LOCAL_PORT}"
    if [ -n "$STORE_IP" ]; then
      FORWARDS+=(-L "127.0.0.1:${STORE_LOCAL_PORT}:${STORE_IP}:${STORE_REMOTE_PORT}")
      echo "→ ${STORE_CONTAINER} ${STORE_IP}:${STORE_REMOTE_PORT} → 127.0.0.1:${STORE_LOCAL_PORT}"
    else
      echo "• ${STORE_CONTAINER} not found — bringing up Postgres only (file/object access disabled)." >&2
    fi
    ...
    if port_open "$PG_LOCAL_PORT"; then
      ...
      [ -n "$STORE_IP" ] && { port_open "$STORE_LOCAL_PORT" \
        && echo "✓ object store tunnel up — S3_ENDPOINT: http://127.0.0.1:${STORE_LOCAL_PORT}" \
        || echo "✗ object store port ${STORE_LOCAL_PORT} didn't open." >&2; }
  ...
  down)
    ...
    echo "✓ closed tunnel (pg ${PG_LOCAL_PORT} + object store ${STORE_LOCAL_PORT}; pids: $(echo "$PIDS" | tr '\n' ' '))"
    ;;
  ...
  status)
    ...
    port_open "$STORE_LOCAL_PORT" \
      && echo "✓ object store tunnel listening on ${STORE_LOCAL_PORT}" \
      || echo "✗ no object store tunnel on ${STORE_LOCAL_PORT}."
    exit $rc
    ;;
