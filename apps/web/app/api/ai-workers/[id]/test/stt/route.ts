...
const Body = z.object({ audioBase64: z.string().min(1), mimeType: z.string().default('audio/webm') });
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await testStt(user.id, id, parsed.data.audioBase64, parsed.data.mimeType));
  } catch (err) {
