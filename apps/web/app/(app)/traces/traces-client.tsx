...
type TracesPage = { traces: TraceSummary[]; total: number; page: number; pageSize: number };

/**
 * Data-free /traces master-detail. The page parses the URL state and hands it
 * here; this fetches the filtered page of traces (GET /api/traces) and the
 * selected trace (GET /api/traces/[id]) via useQuery. Filters/sort/pager/select
 * stay URL-driven `<Link>`s, so navigating re-renders the page → new props →
 * refetch.
 */
export function TracesClient({
  ...
  const listQuery = useQuery({
    queryKey: ['traces', { kinds, statuses, hours, sort, dir, page }],
    queryFn: () => {
      ...
      const q = p.toString();
      return apiFetch<TracesPage>(q ? `/api/traces?${q}` : '/api/traces');
    },
    placeholderData: (prev) => prev,
  });

  const rows = listQuery.data?.traces ?? [];
  ...
  const detailQuery = useQuery({
    ...
    queryFn: () => apiFetch<{ trace: TraceDetailRow }>(`/api/traces/${selectedId}`),
    enabled: !!selectedId,
  });
