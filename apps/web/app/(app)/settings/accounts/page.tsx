...
type Search = { selected?: string; mode?: string; connected?: string; error?: string };
...
export default async function AccountsSettingsPage({
  ...
  const selectedId = showAdd ? null : (sp.selected ?? rows[0]?.id ?? null);
  const selected = rows.find((r) => r.id === selectedId) ?? null;
  const folders = mode === 'folders' && selected ? await listAccountFolders(selected.id) : null;
      ...
      <div className="md:grid md:h-full md:grid-cols-[340px_1fr] md:overflow-hidden">
        ...
        <div className="md:h-full md:min-h-0 md:overflow-y-auto md:scrollbar-thin">
            ...
            <div className="p-4 pb-0">
              {sp.connected && (
                <p className="rounded-md border border-green-500/30 bg-green-50 px-3 py-2 text-sm text-green-900 dark:bg-green-950/40 dark:text-green-100">
                  Connected <span className="font-medium">{sp.connected}</span>. First sync runs
                  within two minutes.
                </p>
              )}
              {sp.error && (
          ...
          {showAdd ? (
              ...
              {folders && folders.ok ? (
                ...
                <div className="space-y-2">
                  <div className="rounded-md border border-destructive/30 bg-destructive/10 px-4 py-3 text-sm text-destructive">
                    Couldn’t list folders: {folders?.error}
                  </div>
                  <Link
                    href={`/settings/accounts?selected=${selected.id}&mode=folders`}
            ...
            <AccountDetail account={selected} latest={latestRuns.get(selected.id)} />
          ) : (
