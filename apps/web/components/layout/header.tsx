...
export function Header({
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 flex h-16 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/60 md:px-6">
      ...
      {pageTitle && (
        <span
          className="pointer-events-none absolute left-1/2 top-1/2 hidden max-w-[40vw] -translate-x-1/2 -translate-y-1/2 truncate px-2 text-center font-logo text-3xl lowercase leading-relaxed text-chart-2 md:block"
          aria-hidden
        >
          {pageTitle}
        </span>
