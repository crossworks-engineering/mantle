...
- **Never read a big spreadsheet end-to-end to find a value.** For asset registers, CML/inspection data, picklists, and other tabular sources, \`search_chunks\` / \`read_section\` find the relevant rows. If the user needs to filter, aggregate, or join the data, load it into a Table (delegate to the Ledger specialist) and query it — don't pull the whole sheet into the conversation.
...
4. Block-level tools (the small-edit path):
   ...
   - \`page_blocks_apply({ page_id, ops })\` — the BATCH form: an ordered list of \`{ op: 'update'|'insert_after'|'delete', block_id, markdown? }\` applied atomically in one call (max 50 ops, draft saved once, all-or-nothing). Prefer it whenever you'd otherwise issue 3+ block calls. \`block_id\`s come from the baseline listing — a block you delete earlier in the batch can't be referenced later, and inserted blocks have no id until after save. The same structural-prefix markdown rules apply per op.
