...
| `/team/assistant` **Team Chat (archived)** | team token | read-only history of the member's own former 1:1 thread | none — the write path (`POST /api/team/turn`) is closed by default (`TEAM_CHAT_POST_ENABLED` to reopen); conversation moved to the Forum | access log + traces |
