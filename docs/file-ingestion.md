...
¹ Documents = `pdf, docx, xlsx, xls, csv, txt, md, json, yaml, dwf, dwg, dxf`. Other types → 415.
...
| `parseDocumentBytes(bytes, ext)` | `@mantle/files` | extractor, `extractAttachmentForTurn` | three-tier dispatch: in-process parsers (pdf-parse/mammoth/exceljs) → Tika fallback → empty string. `.xml` and `.dwf` are the **content-sniffed** branches: a Project plan renders via `@mantle/files/mspdi` (else Tika); a `.dwf` without the DWF magic (usually a renamed DWFx) returns `''` and takes the hollow skip. **Spreadsheets are bounded**: see note below. |
...
| `parseDwf(bytes)` / `parseDwfStructured` / `sniffDwf` | `@mantle/files/dwf` | `parseDocumentBytes` (dwf branch) | Autodesk **DWF** plot sets (ZIP of ePlot sections) → a capped text digest: manifest sheet titles/revisions, per-sheet SOURCE DWG filename + author/layout/paper, layer names scraped from the W2D ASCII opcodes (the circuit registry on circuitization sets), and annotation labels kept by a per-sheet **character budget** (rare tags are the searchable ones — frequency orders, never selects). Routed by **content** (`sniffDwf` gates the extension match; a renamed DWFx yields `''` → the extractor's hollow skip — `isHollowFilenameBody` carves the `dwf` route in because it has no recovery tier). Memory-safe: every entry read preflights the ZIP central directory's declared uncompressed size (same defence as `sheetXmlBytes`), so a deflate bomb degrades a sheet to metadata-only instead of inflating in-process. In-process only — Tika has no DWF handler, so `chars_out: 0` in a `parse_document` trace means "no 2D ePlot sheets", never "service down". Vector geometry is deliberately NOT decoded (that is the source DWG's job; the digest names it per sheet). Live turns cap inline parsing at `DWF_INLINE_PARSE_MAX` (8 MB) and defer bigger sets to the durable extractor. Two derived artifacts per DWF: each sheet becomes an image node via the embedded-images pass (`extractDwfImages`): with the media sidecar up, a REAL raster of the sheet's vector content (ezdwf via `POST /dwf/render`, drawn on a fixed 33-inch page so strokes stay pen-width, ~5400 px on the long edge — engineering-legible down to title-block dates); otherwise the container's 262×170 thumbnail (marked `essential` to pass the 200 px decoration floor), and the auto-table pass builds a **registry workbook** (`parseDwfToGrids`: Sheets / Layers / Labels tabs) so "which sheet carries tag X" is one `table_sql` query. `.dwfx` (OPC/XPS) is a different container → `EXPORT_REQUIRED_EXTS` hint (republish as classic DWF). |
| `parseDwg(bytes)` / `parseDwgToGrids` / `extractDwgImages` / `sniffDwg` | `@mantle/files/dwg` | `parseDocumentBytes` (dwg branch) | AutoCAD **DWG** drawings — the one routed format with NO local parser: one memoised media-sidecar exchange (`POST /dwg/render`, converter chain `dwg2dxf` → `ezdwg`, extraction/render via `ezdxf`) feeds all three passes: the text digest (layers by entity count + character-budgeted annotation labels), the registry workbook (Layers / Texts-with-model-XY / Counts tabs — bake-off drawings carry ZERO DIMENSION entities, so the text layer IS the annotation data), and one essential model-space render image node. Content-sniffed (`AC1xxx` magic; a sniff miss returns `''` → hollow skip, route carved in like `dwf`). No sidecar (or a pre-v0.232.99 image) THROWS a typed error — an honest extract failure, healed by re-queue after enabling the CAD tier; never a filename index. Live turns never parse DWG inline (sidecar round trip) — always deferred to the durable extractor. Render dpi dial: `DWG_RENDER_DPI` (50–600, default 300; an EMPTY value is unset, not 0 — the compose anchor ships it empty and until v0.232.139 that clamped to 50 dpi and the render was then dropped by the image gate as `too_few_bytes`). The sidecar draws on a fixed 33-inch page shaped to the drawing's aspect and clamps the effective dpi under its 30 MP cap (~165 dpi, ~5400 px long edge), and it treats a BLANK render as a failure rather than storing a white rectangle. |
| `parseDxf(bytes)` / `parseDxfToGrids` / `extractDxfImages` / `sniffDxf` | `@mantle/files/dxf` | `parseDocumentBytes` (dxf branch) | AutoCAD **DXF** drawings — the DWG interchange twin, and deliberately ONE code path with it: `@mantle/files/dxf` re-exports thin wrappers from `dwg.ts`, and the SAME memoised `/dwg/render` exchange serves all three passes (the sidecar worker sniffs the bytes and reads a DXF natively with `ezdxf` — reply reports converter `"none"`; a DWG takes the converter chain). Content-sniffed (`sniffDxf`: the 22-byte binary-DXF sentinel, or ASCII group-code pairs opening `0`/`SECTION` with leading `999` comments and a BOM tolerated; a miss returns `''` → hollow skip, route carved in like `dwf`/`dwg`). ASCII DXF is text but stays OUT of `TEXT_EXTS` — raw group codes would index as garbage. Mime is `application/dxf`, NEVER the also-registered `image/vnd.dxf` (ext routing beats client mime on every image path, same trap as `image/vnd.dwg`). Same typed throw without the sidecar, same live-turn deferral, same `DWG_RENDER_DPI` dial. |
...
- **Inline, where the picture belongs**: the agent writes
  ...
  surfaces render through the same TipTap schema, and `jackdaw/lib/rich-markdown.ts`
...
| Commit | What |
|---|---|
| v0.232.86+ | **Autodesk DWF ingestion** (`.dwf` → `INGESTABLE_EXTS`, route `dwf`, parser `@mantle/files/dwf`). Content-sniffed dispatch, declared-size preflight against zip bombs, character-budgeted label digest, capped cross-sheet layer registry, hollow-skip carve-out for the empty parse, `DWF_INLINE_PARSE_MAX` live-turn gate, `.dwfx` → `EXPORT_REQUIRED_EXTS` hint. See the `parseDwf` row in §3. |
| v0.232.99+ | **AutoCAD DWG ingestion** (`.dwg` → `INGESTABLE_EXTS`, route `dwg`, parser `@mantle/files/dwg` over the media sidecar's `/dwg/render`). dwg2dxf (LibreDWG, subprocess-only) primary + ezdwg fallback, ezdxf downstream; digest + registry workbook + essential model-space render; honest typed error without the sidecar. See the `parseDwg` row in §3. |
| v0.232.102+ | **AutoCAD DXF ingestion** (`.dxf` → `INGESTABLE_EXTS`, route `dxf`, `@mantle/files/dxf` — thin re-exports over the DWG machinery). The sidecar's `/dwg/render` worker sniffs its input: DWG magic → the converter chain as before, anything else → native `ezdxf` read (ASCII or binary DXF, converter `"none"`). Everything downstream (registry, caps, render, wiring, failure honesty) is the DWG tier verbatim. No new sidecar *dependency*, but the sniff ships inside the image: DXF needs the **v0.232.102+ media image** — a v0.232.99–101 image runs the DXF through the DWG converter chain and 422s on a valid file. See the `parseDxf` row in §3. |
| v0.232.32 | **`unsupported_media` + `no_parser` dispositions.** Media (`MEDIA_EXTS` or an `audio/*`/`video/*` mime) records an honest terminal skip pointing at `video_ingest`; every OTHER parserless non-image format is caught by the generalised hollow-body guard (`isHollowFilenameBody`, pure + tested in `extractor-parse.ts`) instead of clearing the 20-char check and indexing its own filename as a `success`. Migration 0151 backfills pre-existing media false successes (clears their summary/embedding/chunks, LLM-free re-skip). |
