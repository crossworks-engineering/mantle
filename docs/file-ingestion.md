...
| v0.232.102+ | **AutoCAD DXF ingestion** (`.dxf` → `INGESTABLE_EXTS`, route `dxf`, `@mantle/files/dxf` — thin re-exports over the DWG machinery). The sidecar's `/dwg/render` worker sniffs its input: DWG magic → the converter chain as before, anything else → native `ezdxf` read (ASCII or binary DXF, converter `"none"`). Everything downstream (registry, caps, render, wiring, failure honesty) is the DWG tier verbatim; needs the same v0.232.99+ media image, no new sidecar dependency. See the `parseDxf` row in §3. |
