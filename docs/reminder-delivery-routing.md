...
- Today reminders are **hardcoded to Telegram** and silently dropped if there's
  no allowed Telegram DM ([events-reminders.ts](../server/web/workers/events-reminders.ts)).
  The mobile push pipeline exists but only fires on assistant replies + pending
  approvals ([push-notify.ts](../server/web/workers/push-notify.ts)).
...
- **Blocker found:** the server cannot currently tell a mobile-app message from a
  ...
  ([assistant.ts:240](../server/web/lib/assistant.ts)), even though mobile
  requests are bearer-authenticated and *could* be distinguished
  ([auth.ts `getBearerUser`](../server/web/lib/auth.ts)). There is also no
...
| Concern | Where | Note |
|---|---|---|
| Reminder dispatch | [events-reminders.ts:123-168](../server/web/workers/events-reminders.ts) `tick()` | 30s poll; `findReminderChat()` is Telegram-only; no target ⇒ logged + skipped |
...
| Push send | [notify.ts](../server/web/lib/push/notify.ts) `pushOutbound()`, `pushApproval()` | gated by `assistantMessages`/`approvals` + quiet hours |
| Push worker | [push-notify.ts](../server/web/workers/push-notify.ts) | `LISTEN conversation_changed` + `pending_changed` |
| Device lookup | [store.ts `listSubscriptions`](../server/web/lib/push/store.ts) | by `ownerId` |
| Quiet hours | [quiet-hours.ts `isQuietNow`](../server/web/lib/push/notify.ts), [push.ts:66 `pushPrefs`](../packages/db/src/schema/push.ts) | to be removed |
...
2. Surface "authenticated via mobile bearer" from
   [getSessionUser/getBearerUser](../server/web/lib/auth.ts) up through
   `runAssistantTurn`, and set `channel: 'mobile'` (instead of the hardcoded
   `'web'`) at [assistant.ts:240](../server/web/lib/assistant.ts) when the request
...
In `tick()` ([events-reminders.ts](../server/web/workers/events-reminders.ts)),
...
- Removed `isQuietNow` checks from `pushOutbound`/`pushApproval`
  ([notify.ts](../server/web/lib/push/notify.ts)); deleted `push/quiet-hours.ts`
...
- Drop `quiet_enabled`/`quiet_start`/`quiet_end`/`timezone` from `pushPrefs`
  ...
  ([preferences-sanitize.ts](../server/web/lib/push/preferences-sanitize.ts)) +
...
- Web: a "Reminder delivery" `Select` (Telegram | Mobile app) on
  `/settings/profile` ([profile-client.tsx](../jackdaw/app/(app)/settings/profile/profile-client.tsx)),
  defaulting to the effective value (`reminderChannel ?? 'telegram'`), persisted
  through the existing `updatePreferencesAction`
  ([actions.ts](../jackdaw/app/(app)/settings/profile/actions.ts)), guarded by
  `isReminderChannel` so only valid values are written.
- Mobile API: `GET/PUT /api/profile/reminder-channel`
  ([route.ts](../server/web/app/api/profile/reminder-channel/route.ts)),
...
- ~~Confirm `recordTurn` (agent-runtime) may import profile-preferences
  ...
  **Resolved:** the write lives at the two inbound call sites
  ([assistant.ts](../server/web/lib/assistant.ts) for web/mobile, agent
