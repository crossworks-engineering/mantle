...
| Group → skill travel | `resolveAgentSkills` (`packages/agent-runtime/src/skills.ts`) already unions granted groups' `integration.skillSlug` into an agent's effective skills. `apiSkillSlugForGroup(groupSlug)` = `api-<groupSlug>` is the naming convention. |
