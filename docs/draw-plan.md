...
- The package installs and builds cleanly against client/web (Next 16 +
...
- `pnpm -C client/web add @excalidraw/excalidraw@0.18.1` (exact pin).
- Self-host assets: copy `dist/prod/fonts` into `client/web/public/
...
- `'draw'` into the search enums: `packages/tools/src/builtins.ts`,
  ...
  (`client/web/components/search/node-type-icons.ts`; keyed off the enum so
