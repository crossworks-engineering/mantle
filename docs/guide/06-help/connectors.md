...
Point your brain at an external MCP server and use its tools from your own
agents. Each connector becomes a tool group named `mcp-<slug>`; the server's
tools appear inside it as ordinary tools, and you grant the group to an agent
the same way you grant any other.

This is the outward twin of Settings → MCP. That screen lets other AI clients
reach INTO your brain; this one lets your brain reach OUT to servers other
people run: Firecrawl for ad-hoc scraping, DeepWiki for questions about public
GitHub repositories, or any server speaking streamable HTTP.
...
returns is re-enabled with its history intact. Full detail lives in
