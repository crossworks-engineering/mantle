# Replay: "Remy", the memory-recall agent

> Renamed from "Recall" 2026-08-23: **Recall** now names the memory-map
> system ([`recall.md`](./recall.md)); this feature — replaying what was
> actually said — is **Replay**. The tool is `replay_window`, the groups are
> `replay` / `replay-search` (migration 0154 converged live rows).

How Mantle goes *back in time*. The brain's everyday retrieval
(`search_nodes`, the `entity_*` tools) answers "what do I know about X"
cheaply, one vector query over summaries and facts. **Replay** answers a
...
just out of the live prompt. Replay is the path back to them.
...
Replay is **explicit and pull-based**: it only runs when the user asks to
...
Remy  (agents-table row, depth 2)
   ...
   ├─ replay_window(period_start, period_end)        → raw turns          [message archive]
...
Remy widens or shifts and re-pulls. That iteration is why replay is an agent,
...
[`packages/tools/src/builtins-replay.ts`](../packages/tools/src/builtins-replay.ts),
...
[`builtins-replay.test.ts`](../packages/tools/src/builtins-replay.test.ts).
...
Remy iterating `replay_window` over sub-ranges itself, not by spawning more
...
| `tool_slugs` | `find_window`, `replay_window`, `search_nodes`, `node_read` |
...
`find_window` / `replay_window` as nested steps. Open `/traces` and pivot from
...
- **Dialogue first, working-state on demand.** Recall replays the *exchanged*
  turns by default. Pass `replay_window(..., include_traces: true)` to also fold
...
- **`find_window` sees only digested conversation.** A chat is only digested
  ...
  not have a digest yet. `replay_window` doesn't care, call it directly with a
...
- ~~Recover working-state~~, **shipped**: `replay_window(include_traces: true)`
