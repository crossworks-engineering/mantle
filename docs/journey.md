...
| Write a **Journal** entry | UI / `journal_create` | `journal` | `extractor_run` | **L6 · L5 · L4 · graph** — *plus the deterministic identity block (`buildIdentityContext`) rebuilds from all journal entries and rides every future agent turn (no LLM, no trace — see [`journal.md`](./journal.md))* |
