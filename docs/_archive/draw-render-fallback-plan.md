...
> STATUS: **P0 and P1 BUILT.** Follows [draw-plan.md](_archive/draw-plan.md) and the
> audit in [draw-audit-findings.md](_archive/draw-audit-findings.md). P2 (embedding a
...
**In `server/web`, not `jackdaw`.** Four independent reasons, each verified:
...
present in the image through `jackdaw`. Adding it to `server/web` is a
...
`jackdaw/public/excalidraw-assets/fonts` only; `server/web/public` has no
