...
- **God-functions on hot paths:** `extractNode` (~1,285 lines,
  `server/api/src/agent/extractor.ts`) does skip-gating → body load → chunk →
  embed → facts → entities in one function; and the Telegram turn pipeline in
  `server/api/src/agent/runtime.ts` re-implements `assistant-runtime`'s
...
- **Copy-paste with drift:** `str()`/`strArr()` coercion helpers duplicated 29×
  under `packages/tools/src`; `slugify` duplicated 10× across `server/web` with
