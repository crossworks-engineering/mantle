...
context see [`docs/_archive/fe-be-split-audit.md`](_archive/fe-be-split-audit.md) (the 7-item checklist),
...
- **The arc:** make `server/web` a pure client that talks to the server only over HTTP `/api/**`, so an
...
4. **Minor leftovers** (low priority): `keys-client` dual source of truth (local list + query cache);
   ...
   (only a blocker once `server/api` scales horizontally, see [[api-service-phase1]]).
...
- Auth: `server/web/lib/auth.ts` (`getOwnerOr401`, `getOwnerForAsset`, `buildAssetToken`,
  `detachedDevUser`), `server/web/lib/auth-constants.ts` (`isDetachedDev`, `PUBLIC_PATHS`),
  `server/web/middleware.ts`.
- Client data layer: `server/web/lib/api-fetch.ts` (`apiFetch`/`apiSend`/`apiEventStream`),
  `server/web/lib/asset-url.ts` (`assetUrl`/`setAssetToken`), `components/query-provider.tsx`.
...
- Docs: `docs/_archive/fe-be-split-audit.md` (checklist), `docs/frontend-backend-split.md` (DoD §9),
  `docs/client-data-fetching.md` (recipe), `docs/db-less-dev.md` (detached/DB-less setup),
  `server/web/CLAUDE.md` + `docs/ui-style-guide.md` (UI conventions).
