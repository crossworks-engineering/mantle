...
jackdaw/package.json      dependencies
jackdaw/packages/web-ui/package.json dependencies
```

Those, plus `pnpm-lock.yaml` and the generated `jackdaw/next-env.d.ts`, must
...
`jackdaw/app/(app)/tasks/tasks-client.tsx` → `toggleStatus`.
...
`jackdaw/lib/rank.ts` duplicates `packages/content/src/rank.ts` byte for
...
`CommentThread` into `jackdaw/packages/web-ui` (precedent: `forum-meta.tsx`) with two
