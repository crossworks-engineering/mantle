...
Split `server/web` from a full-stack app into a **pure client + an HTTP API**, to unlock
...
| **Task 3**: DB-less dev | ✅ foundation (v0.61.0). Seam `server/web/lib/remote-data.ts` (`MANTLE_REMOTE_API`). Doc `docs/db-less-dev.md`. |
...
| Task 6, absorb `server/api` into `server/api` | ✅ **done (v0.64.0)**: `server/api` deleted; its runtime + a durable Telegram-turn DBOS workflow now live in `server/api`. |
...
- Client data layer: `server/web/lib/api-fetch.ts` (`apiFetch`/`apiSend`), `components/query-provider.tsx`,
...
- UI conventions (MUST read before UI work): `server/web/CLAUDE.md`, `docs/ui-style-guide.md`.
