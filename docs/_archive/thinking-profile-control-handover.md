...
The whole real-thinking pipeline + GitHub Copilot provider is merged + released (see `docs/_archive/hermes-thinking-and-providers-handover.md`). Today thinking is gated DARK by **env `MANTLE_THINKING_BUDGET`**, read in `packages/agent-runtime/src/tool-loop.ts` via `resolveThinkingBudget()` (env-based). **This task replaces that env gate with the per-user profile gate.**
...
- `server/web/app/api/assistant/turn/route.ts:297`, `isTurnStreamingEnabled() && isStreamThoughtsEnabled(await loadProfilePreferences(user.id))` → 202-stream vs blocking.
- `server/web/app/api/assistant/turn/[turnId]/stream/route.ts:52`, SSE route 404s when off (client falls back to poll).
...
### 2. `server/web/app/api/profile/route.ts`
...
### 3. `server/web/app/(app)/settings/profile/profile-client.tsx`
...
- Update `docs/_archive/hermes-thinking-and-providers-handover.md` §6: thinking is enabled **per user in Settings → Profile** (switch ON + budget), not via env.
...
- `MANTLE_TURN_TOKENS` (server/api), installs the delta observer; if `=0` the loop uses non-streaming `chat()` and NO reasoning streams.
- `MANTLE_TURN_STREAMING` (server/web), the SSE master gate.
...
server/web/app/api/profile/route.ts            :52 zod schema  :74 destructure  :102 save  :24 GET
server/web/app/(app)/settings/profile/profile-client.tsx
...
server/web/app/api/assistant/turn/route.ts:297 / .../[turnId]/stream/route.ts:52  (streamThoughts already gates display)
```
