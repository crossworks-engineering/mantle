...
> run against [draw-audit-handover.md](_archive/draw-audit-handover.md) and
> [draw-plan.md](_archive/draw-plan.md) on 2026-08-07. Method: the handover's own §6
...
- `jackdaw/app/(app)/draw/draws-client.tsx:289`, the list preview, inside the
  **owner's logged-in app session**.
- `jackdaw/packages/web-ui/src/share/draw-presenter.tsx:18`, the public `/s` page, served
...
3. **`pnpm verify` fails today**, and not for an environment reason. `pnpm lint`
   ...
   `jackdaw/app/(app)/draw/draws-client.tsx`, `packages/content/src/draws.ts`,
...
5. **The rebase has a silent trap.** `git merge-tree` confirms exactly two
   ...
   and `jackdaw/components/export/export-menu.tsx`. The trap is in the second:
