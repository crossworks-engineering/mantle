...
manifest's `assistSurface` field ([manifest.ts](../../server/web/lib/system-manifest/manifest.ts) →
`ASSIST_SURFACE_DEFAULTS`); the surface enum lives in
[lib/assist-agent.ts](../server/web/lib/assist-agent.ts) (`AssistSurface =
'pages' | 'tables' | 'apps' | 'dev-tools'`).

| Surface | Agent | Panel (client) | Route | Feedback today |
|---|---|---|---|---|
| **apps** | Appsmith | [app-detail-client.tsx](../server/web/app/(app)/apps/[id]/app-detail-client.tsx) | [ai-assist](../server/web/app/api/apps/[id]/ai-assist/route.ts) | ✅ **live stage labels** (the new reference) |
| **pages** | Pages | [page-editor/ai-assist-panel.tsx](../server/web/components/page-editor/ai-assist-panel.tsx) | [ai-assist](../server/web/app/api/pages/[id]/ai-assist/route.ts) | animated "Pages is working" badge (ping + dots), **no live label** |
| **tables** | Ledger | [table-grid/table-assist-panel.tsx](../server/web/components/table-grid/table-assist-panel.tsx) | [ai-assist](../server/web/app/api/tables/[id]/ai-assist/route.ts) | `Loader2` spinner + "{name} is working…" |
| **dev-tools** | Toolsmith | [dev-tools/assist-panel.tsx](../server/web/components/dev-tools/assist-panel.tsx) | [ai-assist](../server/web/app/api/dev-tools/ai-assist/route.ts) | `Loader2` spinner + "{name} is working, " |

(The **main assistant** (`/assistant`) already has live labels via a separate
path: [use-turn-stage.ts](../server/web/components/assistant/use-turn-stage.ts) →
[GET /api/assistant/turn/stage](../../server/web/app/api/assistant/turn/stage/route.ts) →
[lib/assistant/turn-stage.ts](../../server/web/lib/assistant/turn-stage.ts). That's
...
1. **Stage reader**: [lib/apps/assist-stage.ts](../server/web/lib/apps/assist-stage.ts)
...
2. **Stage endpoint**: [GET /api/apps/[id]/assist-stage](../server/web/app/api/apps/[id]/assist-stage/route.ts)
   returns `{ label }`. `dynamic = 'force-dynamic'`. Owner-scoped.

3. **Poll hook**: [use-assist-stage.ts](../server/web/components/app-sandbox/use-assist-stage.ts):
...
4. **Indicator**: `AppsmithWorking` at the bottom of
   [app-detail-client.tsx](../server/web/app/(app)/apps/[id]/app-detail-client.tsx):
