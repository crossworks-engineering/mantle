...
> Historical design brief: [docs/handover-tools-skills-p6.md](_archive/handover-tools-skills-p6.md).
>
...
1. Web responder, `server/web/lib/assistant.ts:349-360`
2. Telegram responder, `server/api/src/main.ts:897-902`
...
1. **Manifest seeder**: `server/web/lib/system-manifest/seed.ts`.
...
2. **Onboarding**: `server/web/lib/onboarding-provision.ts:~217`. Creates the
...
3. **Boot self-heal**: `server/api/src/main.ts:ensureCoreToolsOnConversationalAgents`
...
4. **Editor**: `server/web/app/(app)/settings/agents/` (server `page.tsx` +
...
Groups (`MANIFEST_TOOL_GROUPS`, `server/web/lib/system-manifest/manifest.ts:~170`):
...
selects the column explicitly). On dev this was safe because `server/api` runs
...
is). Note `server/api`'s prod `start` script is **non-watch** `tsx`.
...
- **Drift test**: `server/web/lib/system-manifest/manifest.test.ts` (run from repo
...
- Manifest / taxonomy / seeding: `server/web/lib/system-manifest/{manifest.ts,seed.ts,integrity.ts}`
- Onboarding: `server/web/lib/onboarding-provision.ts`
- Runtime: `packages/agent-runtime/src/{skills.ts,invoke-agent.ts,tool-loop.ts}`,
  `server/web/lib/assistant.ts`, `server/api/src/main.ts`, `packages/heartbeats/src/{fire.ts,tools.ts,tick.ts}`
...
- Registry/clusters: `packages/tools/src/builtins*.ts` (group membership mirrors
  the `*_TOOLS` clusters)
- Editor/UI: `server/web/app/(app)/settings/agents/{page.tsx,agents-client.tsx}`,
  `/settings/tool-groups`, Studio `server/web/lib/studio/graph.ts`
- Seed CLIs: `server/web/scripts/seed-*.ts` (mostly thin `applyManifest` wrappers;
