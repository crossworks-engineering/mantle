...
(tool loop) + `server/web` (the live trail). Commit map:
...
- **Sink (show it).** `server/web/components/assistant/use-turn-stream.ts` now
...
server/web/components/assistant/use-turn-stream.ts         # accumulate reasoning-delta
server/web/components/assistant/thought-trail.tsx          # collapsible "Thinking" trace
