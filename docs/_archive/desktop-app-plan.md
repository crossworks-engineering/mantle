...
`jackdaw` (staged by `build:ui`, run as a `utilityProcess` on a sticky
...
`jackdaw` (bounce re-asserts presence), plus the shell flushes the cookie
store on window close. Real model turns aren't testable on the e2e stack
(dummy LLM key by design). Nothing ships yet.

## Goal

A desktop app that carries the owner UI (`jackdaw`) out of the browser. First
...
`jackdaw` was carved as a zero-secret client on purpose, and the audit
(2026-07-30) confirms the hard part is already done:

- **No server-side data path.** Zero server actions, zero `/api` proxy routes.
  Every screen is client-fetched through `jackdaw/packages/web-ui/src/api-fetch.ts`,
...
**Bundle the built `jackdaw` inside Electron; talk straight to the brain.**
...
1. **Phase 1: embedded Next standalone.** `next build` (standalone output) run
   ...
   to `jackdaw`; all 16 dynamic `[id]` segments keep working. Cost: ships a
...
the `DesktopBridge` component in `jackdaw` (assistant outbound pings →
...
- **`safeStorage` token vault: shipped.** The bearer at rest moves from
  ...
  exists; no worse than localStorage was). `jackdaw/packages/web-ui/token-store`
