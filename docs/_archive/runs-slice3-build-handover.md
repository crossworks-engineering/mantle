...
- Binding specs, in authority order:
  1. [runs-slice-3-plan.md](_archive/runs-slice-3-plan.md), the AUDITED plan:
  ...
  3. [runs-audit-handover.md](_archive/runs-audit-handover.md) §2, the slices-1+2
...
`server/api/src/workflows/runs-worker-turn.ts` under `withDurableSteps`;
...
claim's stamp. Flag off in server/api → `failed(disabled)`, counter-driving.

**WP2, resume turns as DBOS workflows (0.157.9).**
`server/web/lib/runs/resume.ts` DELETED; body moved to
`server/api/src/workflows/runs-resume-turn.ts`. The two boundaries the plan
...
via `server/api/src/crash-test.ts` `CRASH_TEST_SHAPE=resume`: a registered
...
DATABASE_URL=<scratch-app-db> DBOS_SYSTEM_DATABASE_URL=<scratch-sys-db> \
  ...
  npx tsx server/api/src/crash-test.ts
DATABASE_URL=<same> DBOS_SYSTEM_DATABASE_URL=<same> \
  CRASH_TEST_SHAPE=resume CRASH_MARKER=<m> npx tsx server/api/src/crash-test.ts
```
...
| File | Owns |
|---|---|
| `server/api/src/workflows/runs-worker-turn.ts` | WP1 workflow (whole worker turn; journaled completeItem/requeue; deadline re-stamp; flag refusal) |
| `server/api/src/workflows/runs-resume-turn.ts` | WP2 workflow (both resume modes + panel prompt + telegram delivery + paused refusal) |
| `server/web/lib/runs/dbos-enqueue.ts` | worker-safe DBOSClient seam; workflowID/dedup conventions |
| `server/web/lib/runs/execute-worker.ts` | claim + enqueue + dispatch_failed (shrunk from the slice-2 body) |
| `server/web/workers/runs.ts` | worker/resume lanes now claim-context relays |
...
| `server/web/app/api/debug/runs/[id]/route.ts` + `(app)/debug/runs/runs-client.tsx` | cancel actuator |
| `server/api/src/crash-test.ts` | the resume-shaped gate variant |
