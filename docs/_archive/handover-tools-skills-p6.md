...
- **Tool groups**: `tool_groups` table (mig `0080`), `agents.tool_group_slugs[]`.
  19 default groups in `MANIFEST_TOOL_GROUPS` (`server/web/lib/system-manifest/manifest.ts`).
...
- **Runtime**: `effectiveToolSlugs(agentToolSlugs, groupToolSlugs)` in
  ...
  flattens granted groups. Four call sites: `server/web/lib/assistant.ts` (web),
  `server/api/src/main.ts` (Telegram/responder), `packages/heartbeats/src/fire.ts`,
...
- **Seeding**: `applyManifest` (`server/web/lib/system-manifest/seed.ts`) seeds groups
  + agents decomposed; onboarding (`server/web/lib/onboarding-provision.ts`) seeds the
  persona decomposed. CLI: `pnpm -C server/web seed:tool-groups` (sync group rows,
...
`ensureCoreToolsOnConversationalAgents` (`server/api/src/main.ts`, runs at agent
...
Effective-set diff via a throwaway script under `server/web/scripts/_xxx.ts`
(run `pnpm -C server/web exec tsx --env-file-if-exists=./.env.local scripts/_xxx.ts`,
...
deltas (don't accept surprises). `ALLOWED_USER_ID` is in `server/web/.env.local`.
...
- **Drift test is the gate:** `pnpm exec vitest run system-manifest` from repo root
  (note: run from root, not `-C server/web`).
