...
```
 browser (server/web, Next.js — serves ALL /api/**)        runner (server/api — DBOS, no HTTP)
 ───────────────────────────────────────────────        ──────────────────────────────────
...
**Web routes (`server/web/app/api/assistant/`)**
...
**Client (`server/web/.../assistant/` + `components/assistant/`)**
...
Installed once at boot by `server/api/src/turn-stream-observer.ts`; each is a **no-op unless the trace carries a
...
- **`tsx --watch` on `server/api` does NOT reload workspace-package edits** (`@mantle/voice`, `@mantle/tracing`,
  `@mantle/agent-runtime`, `@mantle/assistant-runtime`). After editing a package, **restart the runner.** Edits
  in `server/api/src` hot-reload.
...
- **Headless scratch scripts** live INSIDE the repo (`server/api/`, `server/web/scripts/`) so pnpm workspace + the
...
- **Prod deploy is a registry pull** (`titanwest/mantle:latest`), not build-on-VPS ([[prod-deploy-is-registry-pull]]);
  run `pnpm -C server/web build` first as a preflight ([[deploy-preflight-next-build]]).
