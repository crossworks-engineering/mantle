...
- **cmdk 1.1.1 is installed** and `server/web/components/ui/command.tsx` wraps
...
**P1, shared icon map.** `server/web/components/search/node-type-icons.ts`:
...
**P2, the palette.** `server/web/components/search/search-palette.tsx`
