...
**Question:** is `server/web`'s frontend now fully separated from the server, talking only over
...
- **Phase 1 runner** (`server/api`, DBOS) and the absorbed agent runtime are intact.
