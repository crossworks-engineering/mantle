...
([runs-audit-handover.md](_archive/runs-audit-handover.md) §8) and the durability ADR
...
resume turns) become DBOS workflows in `server/api`, joining the assistant/
telegram/team/forum turns already running there.

The decisive argument, RESTATED by the audit (the original C1 framing was
wrong for this fleet): server/api pins a STABLE application version
(`applicationVersion: 'mantle-runner-1'`, `server/api/src/config.ts`)
...
|---|---|---|---|
| WP1 | Worker turn execution → DBOS workflow | DBOS (`server/api`) | 1st |
| WP2 | Resume turn execution → DBOS workflow | DBOS (`server/api`) | 2nd |
...
- **C1, version pinning.** In-flight DBOS workflows are pinned to the
  ...
  **VERDICT: PARTIAL.** True of DBOS defaults, startup recovery selects
  ...
  FALSE for this fleet as configured: server/api pins the stable constant
...
- **C2, cross-process enqueue.** `DBOSClient` (the seam in
  `server/web/lib/dbos-client.ts`, used by the Next.js server) also works from
  ...
  *If it falls*: WP1/WP2 route through an internal HTTP endpoint on
  `server/api` instead, same architecture, one more moving part.
  **VERDICT: CONFIRMED** (SDK source). `DBOSClient.enqueue` is a
  ...
  `resolveSystemDatabaseUrl()`; the `dbos` schema must exist (server/api must
  have launched once).
...
- **C4, one engine, unchanged.** WP1/WP2 add NO new engine transitions:
  ...
  `completeItem` + `enqueueRunActionsSafe` itself (server/api imports
...
- **C7, partitioned per-run cap (WP6, deferred).** A DBOS queue with
  ...
  **VERDICT: PLAUSIBLE, unverified**: the `partitionQueue` precedent is
  real (`FORUM_QUEUE`, server/api/src/main.ts). Deferral stands.
...
`server/api/src/workflows/runs-worker-turn.ts`, mirroring
...
(telegram etc.), the workflow runs beside the channel adapters in server/api,
...
2. **Flag discipline across two runtimes**: with `MANTLE_RUNS` off, does
   ...
   flag server-side in server/api, whose env differs from worker_runs.)
...
5. **Two-runtime observability**: a stuck run now spans worker_runs
   (spine) and server/api (turns). Is `trace_ref` + `DBOS.listWorkflows`
   (`server/api/src/runs.ts`) enough to diagnose without SSH archaeology?
...
- **WP1 build items**: on DBOS enqueue rejection, complete the item
  ...
  unqueryable); deploy checklist gains `MANTLE_RUNS` in server/api's env
