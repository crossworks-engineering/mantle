...
[`server/mcp/src/server.ts`](../../server/mcp/src/server.ts) imports only
...
**Today:** `server/mcp` is stdio-only, runs as a child of Claude Desktop/Code,
...
1. Add a transport switch to `server/mcp/src/server.ts`: default stdio
...
2. New authenticated endpoint, e.g. `POST /api/channels/robot/turn`
   ...
   `runAssistantTurn` ([`server/web/lib/assistant.ts:154`](../../server/web/lib/assistant.ts))
...
- **Cleanest:** extract the turn orchestration from `server/web/lib/assistant.ts`
  ...
  the web route and `server/mcp` call one `runResponderTurn()`. This is a
  real refactor; budget for it.
- **Pragmatic:** the MCP server HTTP-calls B1's endpoint (it cannot import
  `server/web`; apps don't import apps). Works day one; adds an internal
  auth hop.
