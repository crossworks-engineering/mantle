...
Prereqs read: [runs.md](../runs.md), [runs-slice-3-plan.md](_archive/runs-slice-3-plan.md)
§4 WP3, `packages/runs/src/{engine,human,sweep}.ts`,
`packages/tools/src/{pending,pending-notify}.ts`,
`server/web/components/assistant/assistant-dock.tsx`, `server/web/lib/realtime.ts`,
...
- Client realtime already exists end-to-end:
  ...
  (`server/web/components/realtime/use-realtime.ts`) is what the sidebar badge
...
- Toast is home-grown (`server/web/components/ui/toast.tsx`, ~80 LOC queue,
  message-only). UI rules: `server/web/CLAUDE.md` + `jackdaw/docs/ui-style-guide.md`
...
3. **Wire it.** `@mantle/tools` registers `notifyPendingCreated` at module
   ...
   must be checked, if `server/web/workers/runs.ts` doesn't already pull in
...
`server/web/components/pending/questionnaire-card.tsx` (client):
...
(`server/web/components/pending/use-pending-questions.ts`): React Query on
...
- **Companion push copy**: `pushApproval` (push-notify worker path) sends a
  ...
  `server/web/lib/push*`/`packages/push*` (wherever `pushApproval` lives).
